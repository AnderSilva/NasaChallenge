...
def main():
    ...
    folder_input  = Path(__file__).parent / "../data/input/"
    folder_output = Path(__file__).parent / "../data/output/"
    ...
    inputs=Path(Path(folder_input) / '*.gz').as_posix()
    files = glob.glob(inputs, recursive=True)
    ...
    #####################################
    filename=Path(folder_output).as_posix()+'/questao_1_hosts_unicos.csv'
    ...
    #####################################
    filename=Path(folder_output).as_posix()+'/questao_2_total_http404.csv'
    ...
    #####################################
    filename=Path(folder_output).as_posix()+'/questao_3_top5_http404.csv'
    ...
    #####################################
    filename=Path(folder_output).as_posix()+'/questao_4_http404_diario.csv'
    df4 = spark.sql("""
            SELECT 
                COUNT(*)           AS Quantidade
    ...
    #####################################
    filename=Path(folder_output).as_posix()+'/questao_5_bytes_retornados.csv'
    df5 = spark.sql("""
            SELECT
